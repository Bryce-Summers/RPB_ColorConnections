...
int main(int argc, char *argv[]) {    
    ...
    while (listen(s0, 1) < 0) {
        ...
        char buffer[1024];
        read(s1, buffer, 1023); 
        /* TODO: find out format of data sent from local laptop (Arduino)
         * NEED: which sensor was triggered */
        int sensor = 0; //TODO: figure out labeling for sensors 
        
        // send data over to main server 
        // Create a socket 
        ...
        //TODO: Fill in the address of bridge server
        sockaddr_in bridgeaddr;
        
        //Connect to bridge server
        connect(client_socket, (struct sockaddr*) &bridgeaddr, sizeof(bridgeaddr));
        char s [1] = {(char) sensor};

        write(client_socket, s, sizeof(int));
        close(client_socket);  
